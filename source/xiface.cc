...
void Xiface::handle_mesg(ITC_mesg *M) {
	...
	case MT_IFC_AUPAR: {
		...
		if (X->_srcid != SRC_GUI_DRAG)
			_audiowin->set_aupar(X);
		break;
	}
	case MT_IFC_DIPAR: {
		M_ifc_dipar *X = (M_ifc_dipar *) M;
		if (X->_srcid != SRC_GUI_DRAG)
			_instrwin->set_dipar(X);
		break;
	}
	case MT_IFC_RETUNE:
...
void Xiface::handle_callb(int k, X_window *W, XEvent *E) {
	...
	case CB_AUDIO_ACT:
		if (_aupar)
			_aupar->_value = _audiowin->value();
		else
			_aupar = new M_ifc_aupar(SRC_GUI_DRAG, _audiowin->asect(),
					_audiowin->parid(), _audiowin->value());
		if (_audiowin->final()) {
	...
	case CB_DIVIS_ACT:
		if (_dipar)
			_dipar->_value = _instrwin->value();
		else
			_dipar = new M_ifc_dipar(SRC_GUI_DRAG, _instrwin->divis(),
					_instrwin->parid(), _instrwin->value());
		if (_instrwin->final()) {
	...
	case CB_RETUNE:
		send_event(TO_MODEL,
				new M_ifc_retune(_instrwin->freq(), _instrwin->temp()));
		break;

	case CB_MIDI_SETCONF:
		send_event(TO_MODEL,
				new M_ifc_chconf(MT_IFC_MCSET, _midiwin->preset(),
						_midiwin->chconf()));
		break;

	case CB_MIDI_GETCONF:
		send_event(TO_MODEL,
				new M_ifc_chconf(MT_IFC_MCGET, _midiwin->preset(), 0));
		break;

	case CB_EDIT_APP:
		send_event(TO_MODEL,
				new M_ifc_edit(MT_IFC_APPLY, _editp->_group, _editp->_ifelm,
						0));
		_mainwin->set_label(_editp->_group, _editp->_ifelm,
				_editp->_synth->_stopname);
		_editwin->lock(true);
		break;
