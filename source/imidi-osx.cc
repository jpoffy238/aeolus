...
Imidi::Imidi(Lfq_u32 *qnote, Lfq_u8 *qmidi, uint16_t *midimap,
		const char *appname) :
		A_thread("Imidi"), _qnote(qnote), _qmidi(qmidi), _midimap(midimap), _appname(
				appname) {
}
...
static void coremidi_proc_static(const MIDIPacketList *pktlist, void *refCon,
		void *connRefCon) {
...
void Imidi::open_midi(void) {
	...
	_handle = NULL;
	CFStringRef name = CFStringCreateWithCString(0, _appname,
...
void Imidi::coremidi_proc(const MIDIPacketList *pktlist, void *refCon,
		void *connRefCon) {
