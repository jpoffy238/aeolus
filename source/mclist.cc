...
X_mclist::X_mclist(X_window *parent, X_callback *callb, X_mclist_style *style,
		int xp, int yp, int xs, int ys, int max_item, int max_char) :
		X_window(parent, xp, yp, xs, ys, style->bg), _style(style), _callb(
				callb), _max_item(max_item), _max_char(max_char), _xs(xs), _ys(
				ys), _nrow(0), _nclm(0) {
...
void X_mclist::expose(XExposeEvent *E) {
	if (_nclm)
		update(E->x, E->y, E->width, E->height);
}
...
void X_mclist::bpress(XButtonEvent *E) {
	...
	for (c = 0; c < _nclm; c++) {
		...
		if ((x > 0) && (x < d)) {
			_sel = _ind[c * _nrow + r];
			if (_sel < _n_item)
				_callb->handle_callb(X_callback::MCLIST | SEL, this,
						(XEvent *) E);
			return;
		}
		x -= d + DX;
			...
			for (r = 0; r < _nrow; r++) {
				...
				if (y + yb + yd > yy) {
					...
					if (x + _ext[j] > xx) {
						...
						XftDrawStringUtf8(D, C, F, x, y + yb,
								(const FcChar8 *) _ptr[j], _len[j]);
					}
				}
				y += dy;
			}
