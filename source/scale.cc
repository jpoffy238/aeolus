...
X_hscale::X_hscale(X_window *parent, X_scale_style *style, int xp, int yp,
		int h, int d) :
		X_window(parent, xp, yp, style->length(), h, style->bg), _style(style), _h(
				h), _d(d) {
...
X_vscale::X_vscale(X_window *parent, X_scale_style *style, int xp, int yp,
		int w, int d) :
		X_window(parent, xp, yp, w, style->length(), style->bg), _style(style), _w(
				w), _d(d) {
	x_add_events(ExposureMask);
}
