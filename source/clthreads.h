...
public:
	...
	enum {
		EM_ALL = ~0, EV_TIME = -1, EV_ERROR = -2
	};

	Esync(void) :
			_event(EV_ERROR), _emask(0) {
...
inline int Esync::eget(unsigned int m, const timespec *t) {
	...
	do {
		if (t)
			r = pthread_cond_timedwait(&_cond, &_mutex, t);
		else
...
public:
	...
	ITC_mesg(unsigned long type = 0) :
			_forw(0), _back(0), _type(type) {
		_counter++;
	}
	virtual ~ITC_mesg(void) {
		_counter--;
	}
	...
	ITC_mesg *forw(void) const {
		return _forw;
	}
	ITC_mesg *back(void) const {
		return _back;
	}
	...
	static size_t object_counter(void) {
		return _counter;
	}
...
public:
	...
	void rem(ITC_mesg *p);
	void flush(void);
	ITC_mesg *head(void) const {
		return _head;
	}
	ITC_mesg *tail(void) const {
		return _tail;
	}
...
inline void ITC_list::rem(ITC_mesg *p) {
	if (p == _head)
		...
		p->_back->_forw = p->_forw;
	if (p == _tail)
		_tail = p->_back;
	else
		p->_forw->_back = p->_back;
	p->_forw = p->_back = 0;
	--_count;
}
...
public:
	...
	enum {
		NO_ERROR = 0, NOT_CONN = 1, DST_LOCK = 2, BAD_PORT = 3
	};
...
public:
	...
	ITC_mesg *get_message(void) const {
		return _mptr;
	}
...
public:
	...
	void get_time(timespec *t) {
		t->tv_sec = _time.tv_sec;
		t->tv_nsec = _time.tv_nsec;
	}
	...
	static void connect(ITC_ctrl *srce, unsigned int opid, Edest *dest,
			unsigned int ipid);
...
inline int ITC_ctrl::find_event(unsigned int mask) const {
	...
	for (b = mask & EM_EC, i = N_EC - 1; b; b <<= 1, i--) {
		if ((b & 0x80000000) && _ecnt[i])
			return i + N_MQ;
	}
	mask <<= N_EC;
	for (b = mask & (EM_MQ << N_EC), i = N_MQ - 1; b; b <<= 1, i--) {
		if ((b & 0x80000000) && _list[i].head())
			return i;
	}
...
public:
	...
	H_thread(Edest *dest, int ipid) :
			_dest(dest), _ipid(ipid) {
	}
	...
	void reply(ITC_mesg *M) {
		_dest->put_event(_ipid, M);
	}
	void reply(void) {
		_dest->put_event(_ipid, 1);
	}
