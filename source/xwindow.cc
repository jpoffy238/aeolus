...
X_window::X_window(X_window *pwin, int xpos, int ypos, int xsize, int ysize,
	...
	_pwin->_list = this;
	_wind = XCreateSimpleWindow(_disp->_dpy, pwin->_wind, xpos, ypos, xsize,
			ysize, bdpix, bdcol, bgcol);
}
...
int X_window::x_set_attrib(unsigned long mask,
		XSetWindowAttributes *attr) const {
...
void X_window::x_apply(X_hints *hints) {
	if (hints->_sh.flags)
		XSetWMNormalHints(_disp->_dpy, _wind, &(hints->_sh));
	if (hints->_mh.flags)
		XSetWMHints(_disp->_dpy, _wind, &(hints->_mh));
