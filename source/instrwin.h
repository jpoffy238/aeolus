...
public:
	...
	Instrwin(X_window *parent, X_callback *callb, int xp, int yp,
			X_resman *xresm);
	~Instrwin(void);
	...
	int divis(void) const {
		return _divis;
	}
	int parid(void) const {
		return _parid;
	}
...
private:
	...
	enum {
		...
		NDIVIS = 8,
		DIVIS_BIT0 = 8,
		DIVIS_STEP = (1 << DIVIS_BIT0),
		DIVIS_MASK = (DIVIS_STEP - 1),
		NTEMPE = 16
	};
	...
	void add_text(int xp, int yp, int xs, int ys, const char *text,
