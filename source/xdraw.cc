...
X_draw::X_draw(Display *dpy, Drawable drw, GC gct, XftDraw *xft) :
		_dpy(dpy), _drw(drw), _gct(gct), _xft_draw(xft), _xft_color(0), _xft_font(
				0), _xx(0), _yy(0) {
	if (_xft_draw && XftDrawDrawable(_xft_draw) != _drw)
		XftDrawChange(_xft_draw, _drw);
}
...
int X_draw::textwidth(const char *str) {
	...
	if (str && (len = strlen(str))) {
		if (_xft_font) {
			...
			return XTextWidth(_x11_font, str, len);
	}
	return 0;
}
...
void X_draw::drawstring(const char *str, int xal) {
	...
	if (str && (len = strlen(str))) {
		if (_xft_font) {
			if (xal >= 0) {
				XftTextExtentsUtf8(_dpy, _xft_font, (const FcChar8 *) str, len,
						&G);
				dx = G.width;
				if (xal == 0)
					dx /= 2;
			}
			XftDrawStringUtf8(_xft_draw, _xft_color, _xft_font, _xx - dx, _yy,
