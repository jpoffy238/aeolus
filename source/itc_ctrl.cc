...
unsigned long ITC_ctrl::delay(void) {
	...
	clock_gettime(CLOCK_REALTIME, &t);
	return (t.tv_nsec - _time.tv_nsec) / 1000
			+ 1000000 * (t.tv_sec - _time.tv_sec);
#endif
}
...
int ITC_ctrl::send_event(unsigned int opid, ITC_mesg *M) {
	if ((opid < N_OP) && _dest[opid])
		...
		return Edest::NOT_CONN;
}

int ITC_ctrl::send_event(unsigned int opid, unsigned int incr) {
	if ((opid < N_OP) && _dest[opid])
		return _dest[opid]->put_event(_ipid[opid], incr);
	else
		return Edest::NOT_CONN;
}

void ITC_ctrl::connect(ITC_ctrl *srce, unsigned int opid, Edest *dest,
		unsigned int ipid) {
	assert(srce);
	assert(opid < N_OP);
