...
Audio::Audio(const char *name, Lfq_u32 *qnote, Lfq_u32 *qcomm) :
		A_thread("Audio"), _appname(name), _qnote(qnote), _qcomm(qcomm), _qmidi(
				0), _running(false),
#ifdef __linux__
				_alsa_handle (0),
#endif
				_jack_handle(0), _abspri(0), _relpri(0), _bform(0), _nplay(0), _fsamp(
						0), _fsize(0), _nasect(0), _ndivis(0) {
}
...
void Audio::init_jack(const char *server, bool bform, Lfq_u8 *qmidi) {
	...
	if (server)
		opts |= JackServerName;
	_jack_handle = jack_client_open(_appname, (jack_options_t) opts, &stat,
			server);
	if (!_jack_handle) {
	...
	jack_set_process_callback(_jack_handle, jack_static_callback,
			(void *) this);
	...
	for (i = 0; i < _nplay; i++) {
		_jack_opport[i] = jack_port_register(_jack_handle, p[i],
	...
	if (_qmidi) {
		_jack_midipt = jack_port_register(_jack_handle, "Midi/in",
...
void Audio::proc_mesg(void) {
	...
	while (get_event_nowait() != EV_TIME) {
		...
		case MT_LOAD_RANK: {
			...
			_divisp[X->_divis]->set_rank(X->_rank, X->_wave, X->_sdef->_pan,
					X->_sdef->_del);
			send_event(TO_MODEL, M);
			M = 0;
			break;
