...
public:
	...
	unsigned long blackpixel(void) const {
		return BlackPixel (_dpy, _dsn);
	}
	unsigned long whitepixel(void) const {
		return WhitePixel (_dpy, _dsn);
	}

	Display *dpy(void) const {
		return _dpy;
	}
	int dsn(void) const {
		return _dsn;
	}
	Colormap dcm(void) const {
		return _dcm;
	}
	GC dgc(void) const {
		return _dgc;
	}
	Visual *dvi(void) const {
		return _dvi;
	}
	XftDraw *xft(void) const {
		return _xft;
	}
	XIM xim(void) const {
		return _xim;
	...
	int depth(void) const {
		return DefaultDepth (_dpy, _dsn);
	}
	int xsize(void) const {
		return WidthOfScreen (ScreenOfDisplay (_dpy, _dsn));
	}
	int ysize(void) const {
		return HeightOfScreen (ScreenOfDisplay (_dpy, _dsn));
	}
	int gray(void) const {
		return 0;
	}
	...
	void nofocus(void) {
		XSetInputFocus(_dpy, PointerRoot, PointerRoot, CurrentTime);
	}
...
public:
	...
	void rname(const char *rname) {
		_ch.res_name = (char *) rname;
	}
	void rclas(const char *rclas) {
		_ch.res_class = (char *) rclas;
	}
...
public:
	...
	int x_map(void) const {
		return XMapWindow(_disp->_dpy, _wind);
	}
	int x_mapraised(void) const {
		return XMapRaised(_disp->_dpy, _wind);
	}
	int x_unmap(void) const {
		return XUnmapWindow(_disp->_dpy, _wind);
	}
	int x_clear(void) const {
		return XClearWindow(_disp->_dpy, _wind);
	}
	int x_raise(void) const {
		return XRaiseWindow(_disp->_dpy, _wind);
	}
...
public:
	...
	void init(int *argc, char *argv[], char *name, XrmOptionDescRec *opt,
			int nopt);

	const char *get(const char *res, const char *def) const;
	int getb(const char *res, int def) const;
	void geometry(const char *res, int xd, int yd, int bd, int &xp, int &yp,
			int &xs, int &ys);

	const char *rname(void) {
		return _rname;
	}
	const char *rclas(void) {
		return _rclas;
	}
...
public:
	...
	static int cb_class(int k) {
		return k & 0xFFFFFF00;
	}
	static int cb_event(int k) {
		return k & 0x000000FF;
	}
};
...
public:
	...
	void setcolor(unsigned long c) {
		_xft_color = 0;
		XSetForeground(_dpy, _gct, c);
	}
	void setcolor(XftColor *c) {
		_xft_color = c;
		XSetForeground(_dpy, _gct, c->pixel);
	}
	void setfunc(int f) {
		XSetFunction(_dpy, _gct, f);
	}
	void setline(int d) {
		XSetLineAttributes(_dpy, _gct, d, LineSolid, CapButt, JoinBevel);
	}
	void setline(int d, int m) {
		XSetLineAttributes(_dpy, _gct, d, m, CapButt, JoinBevel);
	}
	void setdash(char *v, int n, int i = 0) {
		XSetDashes(_dpy, _gct, i, v, n);
	}
	...
	void draw(int x, int y) {
		XDrawLine(_dpy, _drw, _gct, _xx, _yy, x, y);
		_xx = x;
		_yy = y;
	}
	void rdraw(int x, int y) {
		XDrawLine(_dpy, _drw, _gct, _xx, _yy, _xx + x, _yy + y);
		_xx += x;
		_yy += y;
	}
	void drawline(int x0, int y0, int x1, int y1) {
		XDrawLine(_dpy, _drw, _gct, x0, y0, x1, y1);
	}
	void drawrect(int x0, int y0, int x1, int y1) {
		XDrawRectangle(_dpy, _drw, _gct, x0, y0, x1 - x0, y1 - y0);
	}
	void fillrect(int x0, int y0, int x1, int y1) {
		XFillRectangle(_dpy, _drw, _gct, x0, y0, x1 - x0, y1 - y0);
	}
	void drawrect(int dx, int dy) {
		XDrawRectangle(_dpy, _drw, _gct, _xx, _yy, dx, dy);
	}
	void fillrect(int dx, int dy) {
		XFillRectangle(_dpy, _drw, _gct, _xx, _yy, dx, dy);
	}
	void drawarc(int dx, int dy, int a1, int a2) {
		XDrawArc(_dpy, _drw, _gct, _xx, _yy, dx, dy, a1, a2);
	}
	void fillarc(int dx, int dy, int a1, int a2) {
		XFillArc(_dpy, _drw, _gct, _xx, _yy, dx, dy, a1, a2);
	}
	void movepix(int dx, int dy, int xs, int ys);
	void setclip(int x0, int y0, int x1, int y1);
	void noclip(void) {
		XSetClipMask(_dpy, _gct, None);
	}
	void drawpoints(int n, XPoint *P);
	void drawlines(int n, XPoint *P);
	void drawsegments(int n, XSegment *S);
	void putimage(XImage *I) {
		XPutImage(_dpy, _drw, _gct, I, 0, 0, _xx, _yy, I->width, I->height);
	}
	void clearwin(void) {
		XClearWindow(_dpy, _drw);
	}

	int textwidth(const char *str);
	int textascent(void) {
		return _xft_font ?
				_xft_font->ascent : (_x11_font ? _x11_font->ascent : 0);
	}
	int textdescent(void) {
		return _xft_font ?
				_xft_font->descent : (_x11_font ? _x11_font->descent : 0);
	}
...
public:
	...
	enum {
		PLAIN = 0, BORDER = 1, RAISED = 2, ALEFT = 4, ARIGHT = 8
	};
...
protected:
	...
	X_button(X_window *parent, X_callback *callb, X_button_style *style, int xp,
			int yp, int cbid);
...
public:
	...
	X_tbutton(X_window *parent, X_callback *callb, X_button_style *style,
			int xp, int yp, const char *text1 = 0, const char *text2 = 0,
			int cbid = 0);
	...
	void set_col(unsigned long bg, XftColor *fg) {
		_bg = bg, _fg = fg;
	}
	void set_text(const char *txt1, const char *txt2);
	const char *text1(void) const {
		return _text1;
	}
	const char *text2(void) const {
		return _text2;
	}
...
public:
	...
	X_ibutton(X_window *parent, X_callback *callb, X_button_style *style,
			int xp, int yp, XImage *image, int cbid = 0);

	~X_ibutton(void);

	virtual void redraw(void);

	void set_col(unsigned long bg, unsigned long fg) {
		_bg = bg, _fg = fg;
	}
	void set_image(XImage *image) {
		_image = image;
	}
	const XImage *image(void) const {
		return _image;
	}
...
public:
	...
	X_pbutton(X_window *parent, X_callback *callb, X_button_style *style,
			int xp, int yp, Pixmap map0, Pixmap map1, int cbid = 0);

	~X_pbutton(void);

	virtual void set_stat(bool stat);

	const Pixmap map0(void) const {
		return _map0;
	}
	const Pixmap map1(void) const {
		return _map1;
	}
...
public:
	...
	enum {
		ENDGAP = 1, PLUSMIN = 2, MIDGAP = 4, ARROWS = 8
	};
	enum {
		MOVE,
		STOP,
	...
	X_scroll(X_window *parent, X_callback *callb, X_button_style *style, int xp,
			int yp, int xs, int ys, int opts);

	~X_scroll(void);

	float offs(void) const {
		return _offs;
	}
	float frac(void) const {
		return _frac;
	}
...
public:
	...
	X_textln(X_window *parent, X_textln_style *style, int xp, int yp, int xs,
			int ys, const char *text, int xal);
...
public:
	...
	X_textip(X_window *parent, X_callback *callb, X_textln_style *style, int xp,
			int yp, int xs, int ys, int max, X_linked *back = 0,
			X_linked *forw = 0);
	...
	X_linked *forw(void) const {
		return _forw;
	}
	X_linked *back(void) const {
		return _back;
	}
...
private:
	...
	enum {
		FOCUS = 1, CLEAR = 2, CALLB = 4, RIGHT = 8, CENTER = 16
	};
...
public:
	...
	X_enumip(X_window *parent, X_callback *callb, X_textln_style *style, int xp,
			int yp, int xs, int ys, X_enip_item *list, X_linked *back = 0,
			X_linked *forw = 0);
...
public:
	...
	enum {
		PLAIN = 0, BORDER = 1, RAISED = 2, PDOWN = 256
	};
...
public:
	...
	enum {
		MAXITEM = 32, DL = 6, DR = 6
	};

	X_menuwin(X_window *parent, X_callback *callb, X_menuwin_style *style,
			int xp, int yp, X_menuwin_item *items);
...
public:
	...
	void set_tick(int i, int p, float v, const char *t) {
		pix[i] = p;
		val[i] = v;
		text[i] = t;
	}
	void set_text(int i, const char *t) {
		text[i] = t;
	}
	int pixlen(void) const {
		return pix[0] + pix[nseg] + 1;
	}
	int length(void) const {
		return 2 * marg + pixlen();
	}
	int limit(int i);
	int calcpix(float x);
	float calcval(int p);
...
public:
	...
	X_hscale(X_window *parent, X_scale_style *style, int xp, int yp, int h,
			int d = 0);
...
public:
	...
	X_vscale(X_window *parent, X_scale_style *style, int xp, int yp, int w,
			int d = 0);
...
public:
	...
	X_meter(X_window *parent, X_meter_style *style, X_scale_style *scale,
...
public:
	...
	X_hmeter(X_window *parent, X_meter_style *style, X_scale_style *scale,
			int xp, int yp, int h);
...
public:
	...
	X_vmeter(X_window *parent, X_meter_style *style, X_scale_style *scale,
			int xp, int yp, int w);
...
public:
	...
	X_slider(X_window *parent, X_callback *callb, X_slider_style *style,
			X_scale_style *scale, int xp, int yp, int xs, int ys, int cbid = 0);

	virtual void set_val(float v) = 0;
	virtual void set_color(unsigned long knob, unsigned long mark) = 0;
	float get_val(void) {
		return _scale->calcval(_i);
	}
	int cbid(void) const {
		return _cbid;
	}
...
public:
	...
	X_hslider(X_window *parent, X_callback *callb, X_slider_style *style,
			X_scale_style *scale, int xp, int yp, int h, int cbid);
...
public:
	...
	X_vslider(X_window *parent, X_callback *callb, X_slider_style *style,
			X_scale_style *scale, int xp, int yp, int w, int cbid);
...
public:
	...
	X_mclist(X_window *parent, X_callback *callb, X_mclist_style *style, int xp,
			int yp, int xs, int ys, int max_item, int max_char);
