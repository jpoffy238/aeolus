...
Midimatrix::Midimatrix(X_window *parent, X_callback *callb, int xp, int yp) :
		X_window(parent, xp, yp, 100, 100, Colors.midi_bg), _callb(callb), _mapped(
				false) {
...
void Midimatrix::plot_conn(int x, int y) {
	...
	if (y < _nkeybd)
		D.setcolor(Colors.midi_bg ^ Colors.midi_co1);
	else if (y < _nkeybd + _ndivis)
		D.setcolor(Colors.midi_bg ^ Colors.midi_co2);
	else
		D.setcolor(Colors.midi_bg ^ Colors.midi_co3);
	D.setfunc(GXxor);
	x = XL + x * DX + 5;
	y = YT + y * DY + 5;
...
void Midimatrix::bpress(XButtonEvent *E) {
	...
	x = E->x - XL - 4 - i * DX;
	y = E->y - YT - 4 - j * DY;
	if ((i > 15) || ((int) j > _nkeybd + _ndivis) || (x > DX - 2)
			|| (y > DY - 2))
		return;
	_chan = i;
	...
	if (_callb)
		_callb->handle_callb(CB_MIDI_MODCONF, this, 0);
}
