...
void H_scale::handle_event(XEvent *E) {
	...
	case ButtonPress:
		...
		x -= 12 * _i + 6;
		if (_callb && (abs(x) < 6))
			_callb->handle_callb(CB_SC_HARM, this, E);
		break;
	}
}
...
void N_scale::handle_event(XEvent *E) {
	...
	case ButtonPress:
		...
		x -= _i * FW_DX + FW_DX / 2;
		if (_callb && (abs(x) < 10))
			_callb->handle_callb(CB_SC_NOTE, this, E);
		break;
	}
}
...
const char *Editwin::_pftb_text[N_PFTB] = { "32", "16", "10 2/3", "8", "5 1/3",
...
Editwin::Editwin(X_window *parent, X_callback *callb, int xp, int yp,
		X_resman *xresm) :
		X_window(parent, xp, yp, 100, 100, Colors.main_bg), _callb(callb), _xresm(
				xresm) {
	int i, x, y;
	X_hints H;
	X_window *W;
	...
	but1.size.x = 150;
	but1.size.y = 20;
	_tabb[0] = new X_tbutton(this, this, &but1, 0, 0, "General / Vol / Tune", 0,
			TAB_GEN);
	_tabb[1] = new X_tbutton(this, this, &but1, 150, 0, "Harmonics - Levels", 0,
			TAB_LEV);
	_tabb[2] = new X_tbutton(this, this, &but1, 300, 0, "Harmonics - Attack", 0,
			TAB_ATT);
	_tabb[3] = new X_tbutton(this, this, &but1, 450, 0, "Harmonics - Random", 0,
			TAB_RAN);
	for (i = 0; i < 4; i++)
		_tabb[i]->x_map();
	but1.size.x = 80;
	...
	but1.size.x = 60;
	for (i = 0; i < N_PFTB; i++) {
		_pftb[i] = new X_tbutton(W, this, &but1, x, y, _pftb_text[i], 0,
				B_PFTB + i);
		_pftb[i]->x_map();
		x += 60;
	}
	...
	_vol_fun = new Functionwin(W, this, x + 28, y + 20, Colors.func_bg,
			Colors.func_gr, Colors.func_mk);
	...
	_tun_fun = new Functionwin(W, this, x + 28, y + 20, Colors.func_bg,
			Colors.func_gr, Colors.func_mk);
	...
	_atu_fun = new Functionwin(W, this, x + 28, y + 20, Colors.func_bg,
			Colors.func_gr, Colors.func_mk);
	...
	_dtu_fun = new Functionwin(W, this, x + 28, y + 20, Colors.func_bg,
			Colors.func_gr, Colors.func_mk);
	...
	y += 335;
	_lev_fun = new Functionwin(W, this, 28, y, Colors.func_bg, Colors.func_gr,
			Colors.func_mk);
	...
	y += 230;
	_att_fun = new Functionwin(W, this, 28, y, Colors.func_bg, Colors.func_gr,
			Colors.func_mk);
	...
	y += 230;
	_ran_fun = new Functionwin(W, this, 28, y, Colors.func_bg, Colors.func_gr,
			Colors.func_mk);
...
void Editwin::handle_callb(int k, X_window *W, XEvent *E) {
	...
	case CB_MS_UND: {
		...
		switch (_ctab) {
		case TAB_LEV:
			msl_update(&_edit->_h_lev, _lev_msl, _lev_fun, 0, d, _lev_harm,
					_lev_note);
			break;
		case TAB_ATT:
			if (M == _att_msl)
				msl_update(&_edit->_h_att, _att_msl, _att_fun, 0, d, _att_harm,
						_att_note);
			else
				msl_update(&_edit->_h_atp, _atp_msl, _att_fun, 1, d, _att_harm,
						_att_note);
			break;
		case TAB_RAN:
			msl_update(&_edit->_h_ran, _ran_msl, _ran_fun, 0, d, _ran_harm,
					_ran_note);
			break;
		}
		_save->set_stat(1);
	...
	case CB_FW_UND: {
		...
		case TAB_GEN:
			if (F == _vol_fun) {
				if (f)
					fun_update(&_edit->_n_ins, F, d);
				else
					fun_update(&_edit->_n_vol, F, d);
			} else if (F == _tun_fun) {
				if (f)
					fun_update(&_edit->_n_ran, F, d);
				else
					fun_update(&_edit->_n_off, F, d);
			} else if (F == _atu_fun) {
				if (f)
					fun_update(&_edit->_n_atd, F, d);
				else
					fun_update(&_edit->_n_att, F, d);
			} else if (F == _dtu_fun) {
				if (f)
					fun_update(&_edit->_n_dcd, F, d);
				else
					fun_update(&_edit->_n_dct, F, d);
			}
			break;
		case TAB_LEV:
			fun_update(&_edit->_h_lev, _lev_msl, _lev_fun, d, _lev_harm,
					_lev_note);
			break;
		case TAB_ATT:
			if (f)
				fun_update(&_edit->_h_atp, _atp_msl, _att_fun, d, _att_harm,
						_att_note);
			else
				fun_update(&_edit->_h_att, _att_msl, _att_fun, d, _att_harm,
						_att_note);
			break;
		case TAB_RAN:
			fun_update(&_edit->_h_ran, _ran_msl, _ran_fun, d, _ran_harm,
					_ran_note);
			break;
		}
		_save->set_stat(1);
...
void Editwin::add_text(X_window *win, int xp, int yp, int xs, int ys,
