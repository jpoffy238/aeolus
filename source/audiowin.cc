...
Audiowin::Audiowin(X_window *parent, X_callback *callb, int xp, int yp,
		X_resman *xresm) :
		X_window(parent, xp, yp, 200, 100, Colors.main_bg), _callb(callb), _xresm(
				xresm), _xp(xp), _yp(yp) {
...
void Audiowin::setup(M_ifc_init *M) {
	...
	for (i = 0; i < _nasect; i++) {
		...
		(S->_slid[0] = new X_hslider(this, this, &sli1, &sca_azim, x, 40, 20,
				k + 0))->x_map();
		(S->_slid[1] = new X_hslider(this, this, &sli1, &sca_difg, x, 75, 20,
				k + 1))->x_map();
		(S->_slid[2] = new X_hslider(this, this, &sli1, &sca_dBsh, x, 110, 20,
				k + 2))->x_map();
		(S->_slid[3] = new X_hslider(this, this, &sli1, &sca_dBsh, x, 145, 20,
				k + 3))->x_map();
		(S->_slid[4] = new X_hslider(this, this, &sli1, &sca_dBsh, x, 180, 20,
				k + 4))->x_map();
...
void Audiowin::add_text(int xp, int yp, int xs, int ys, const char *text,
