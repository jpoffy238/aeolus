...
X_meter::X_meter(X_window *parent, X_meter_style *style, X_scale_style *scale,
		...
		X_window(parent, xp, yp, xs, ys, style->bg), _style(style), _scale(
				scale), _x(scale->marg), _y(scale->marg), _d(0) {
	_d0 = _i = -1;
...
void X_meter::plotmarks(int a, int b, unsigned long c) {
	...
	k = _scale->nseg + 1;
	p = _scale->pix;
	if (c == _style->act.c0 || c == _style->act.c1)
		XSetForeground(dpy(), dgc(), _style->act.mk);
	else
		XSetForeground(dpy(), dgc(), _style->off.mk);
	while (k--) {
		i = *p++;
		if (i > b)
			break;
