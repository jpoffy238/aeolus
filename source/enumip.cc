...
X_enumip::X_enumip(X_window *parent, X_callback *callb, X_textln_style *style,
		int xp, int yp, int xs, int ys, X_enip_item *list, X_linked *back,
		X_linked *forw) :
		X_window(parent, xp, yp, xs, ys, style->color.normal.bgnd), X_linked(
				back, forw), _callb(callb), _style(style), _list(list), _bg(
				_style->color.normal.bgnd), _fg(_style->color.normal.text), _txt(
				0), _ind(0), _len(0), _foc(0), _xs(xs), _ys(ys), _x0(xs - DX) {
	int i, l, m;
...
void X_enumip::set_ind(int ind) {
	...
	_x0 = _xs - DX;
	if (_len)
		_x0 -= textwidth(0, _len);
	redraw();
}

void X_enumip::select(void) {
	if (_callb)
		_callb->handle_callb(X_callback::ENUMIP | BUT, this, 0);
}
...
void X_enumip::cbkey(int k) {
	_key = k;
	if (_callb)
		_callb->handle_callb(X_callback::ENUMIP | KEY, this, 0);
}
