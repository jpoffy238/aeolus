...
X_scroll::X_scroll(X_window *parent, X_callback *callb, X_button_style *style,
		int xp, int yp, int xs, int ys, int opts) :
		X_window(parent, xp, yp, xs, ys, style->color.shadow.bgnd), _style(
				style), _callb(callb), _xs(xs), _ys(ys), _km(0), _k0(0), _dk(0), _dw(
				0), _zz(INT_MIN), _offs(0.0f), _frac(1.0f) {
	X_ibutton *B;
	...
	if (xs >= 2 * ys) {
		...
		if (opts & PLUSMIN) {
			_km -= DL;
			B = new X_ibutton(this, this, style, _km, _dw,
					disp()->image1515(X_display::IMG_MI),
					X_callback::SCROLL | B1MI);
			B->x_set_win_gravity(NorthEastGravity);
			B->x_map();
			_km -= DL;
			B = new X_ibutton(this, this, style, _km, _dw,
					disp()->image1515(X_display::IMG_PL),
					X_callback::SCROLL | B1PL);
			B->x_set_win_gravity(NorthEastGravity);
			B->x_map();
		}
		if (opts & MIDGAP)
		...
		if (opts & ARROWS) {
			_km -= DL;
			B = new X_ibutton(this, this, style, _km, _dw,
					disp()->image1515(X_display::IMG_RT),
					X_callback::SCROLL | B1RD);
			B->x_set_win_gravity(NorthEastGravity);
			B->x_map();
			_km -= DL;
			B = new X_ibutton(this, this, style, _km, _dw,
					disp()->image1515(X_display::IMG_LT),
					X_callback::SCROLL | B1LU);
			B->x_set_win_gravity(NorthEastGravity);
			B->x_map();
		}
	} else if (ys >= 2 * xs) {
		...
		if (opts & PLUSMIN) {
			_km -= DL;
			B = new X_ibutton(this, this, style, _dw, _km,
					disp()->image1515(X_display::IMG_MI),
					X_callback::SCROLL | B1MI);
		...
		if (opts & ARROWS) {
			_km -= DL;
			B = new X_ibutton(this, this, style, _dw, _km,
					disp()->image1515(X_display::IMG_DN),
					X_callback::SCROLL | B1RD);
			B->x_set_win_gravity(SouthWestGravity);
			B->x_map();
			_km -= DL;
			B = new X_ibutton(this, this, style, _dw, _km,
					disp()->image1515(X_display::IMG_UP),
					X_callback::SCROLL | B1LU);
			B->x_set_win_gravity(SouthWestGravity);
			B->x_map();
		}
...
void X_scroll::bpress(XButtonEvent *E) {
	...
	if (k >= _dk)
		_callb->handle_callb(
				X_callback::SCROLL | ((E->button == Button3) ? C3RD : C1RD),
				this, 0);
	else if (k < 0)
		_callb->handle_callb(
				X_callback::SCROLL | ((E->button == Button3) ? C3LU : C1LU),
				this, 0);
	else
		_zz = k + _k0;
}
...
void X_scroll::redraw(void) {
	...
	if (_xs > _ys) {
		...
		XFillRectangle(dpy(), win(), dgc(), _k0 + _dk, 0, _km - _k0 - _dk + 1,
				_ys);
		...
		XDrawLine(dpy(), win(), dgc(), _k0 + _dk, _dw + DW - 1, _k0 + 2,
				_dw + DW - 1);
		XDrawLine(dpy(), win(), dgc(), _k0 + _dk, _dw + DW - 1, _k0 + _dk,
				_dw + 1);
	} else {
		XSetForeground(dpy(), dgc(), _style->color.shadow.bgnd);
		XFillRectangle(dpy(), win(), dgc(), 0, 0, _xs, _k0 + 2);
		XFillRectangle(dpy(), win(), dgc(), 0, _k0 + _dk, _xs,
				_km - _k0 - _dk + 1);
		XSetForeground(dpy(), dgc(), _style->color.bg[1]);
		...
		XSetForeground(dpy(), dgc(), _style->color.shadow.dark);
		XDrawLine(dpy(), win(), dgc(), _dw + DW - 1, _k0 + _dk, _dw + DW - 1,
				_k0 + 2);
		XDrawLine(dpy(), win(), dgc(), _dw + DW - 1, _k0 + _dk, _dw + 1,
				_k0 + _dk);
	}
}
...
void X_scroll::handle_callb(int type, X_window *W, XEvent *E) {
	...
	if (_callb)
		_callb->handle_callb(B->cbid() + ((Z->button == Button3) ? 1 : 0), this,
				E);
}
