...
Model::Model(Lfq_u32 *qcomm, Lfq_u8 *qmidi, uint16_t *midimap,
		const char *appname, const char *stops, const char *instr,
		const char *waves, bool uhome) :
		A_thread("Model"), _qcomm(qcomm), _qmidi(qmidi), _midimap(midimap), _appname(
				appname), _stops(stops), _uhome(uhome), _ready(false), _nasect(
				0), _ndivis(0), _nkeybd(0), _ngroup(0), _count(0), _bank(0), _pres(
				0), _sc_cmode(0), _sc_group(0), _audio(0), _midi(0) {
...
void Model::proc_qmidi(void) {
	...
	while (_qmidi->read_avail() >= 3) {
		...
		case 0xB0:
			...
			case MIDICTL_SWELL:
				// Swell pedal
				set_dipar(SRC_MIDI_PAR, d, 0,
						SWELL_MIN + v * (SWELL_MAX - SWELL_MIN) / 127.0f);
				break;

			case MIDICTL_TFREQ:
				// Tremulant frequency
				set_dipar(SRC_MIDI_PAR, d, 1,
						TFREQ_MIN + v * (TFREQ_MAX - TFREQ_MIN) / 127.0f);
				break;

			case MIDICTL_TMODD:
				// Tremulant amplitude
				set_dipar(SRC_MIDI_PAR, d, 2,
...
void Model::init_iface(void) {
	...
	for (j = 0; j < 4; j++) {
		send_event(TO_IFACE,
				new M_ifc_aupar(0, -1, j, _audio->_instrpar[j]._val));
	}
	for (i = 0; i < _nasect; i++) {
		for (j = 0; j < 5; j++) {
			send_event(TO_IFACE,
					new M_ifc_aupar(0, i, j, _audio->_asectpar[i][j]._val));
		}
	}
	for (i = 0; i < _ndivis; i++) {
		for (j = 0; j < 3; j++) {
			send_event(TO_IFACE,
					new M_ifc_dipar(0, i, j, _divis[i]._param[j]._val));
		}
	}
...
void Model::set_state(int bank, int pres) {
	...
	if (get_preset(bank, pres, d)) {
		...
		send_event(TO_IFACE,
				new M_ifc_preset(MT_IFC_PRRCL, bank, pres, _ngroup, d));
	} else
		send_event(TO_IFACE, new M_ifc_preset(MT_IFC_PRRCL, bank, pres, 0, 0));
}
		...
		*q++ = 0;
		...
		if (!strcmp(p, "/instr/new")) {
			...
			if (D || G)
				...
				if (_nkeybd == NKEYBD) {
					fprintf(stderr,
							"Line %d: can't create more than %d keyboards\n",
							line, NKEYBD);
					stat = ERROR;
			...
			if (D || G)
				stat = BAD_SCOPE;
			else if (sscanf(q, "%f%d%n", &_fbase, &_itemp, &n) != 2)
				stat = ARGS;
			else
				q += n;
...
int Model::read_presets(void) {
	...
	if (_uhome) {
		...
		if (p)
			sprintf(name, "%s/.aeolus-presets", p);
		else
			strcpy(name, ".aeolus-presets");
	} else {
