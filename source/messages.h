...
public:
	...
	M_ifc_ifelm(int type, int g, int i) :
			ITC_mesg(type), _group(g), _ifelm(i) {
	}
...
public:
	...
	M_ifc_aupar(int s, int a, int p, float v) :
			ITC_mesg(MT_IFC_AUPAR), _srcid(s), _asect(a), _parid(p), _value(v) {
	}
...
public:
	...
	M_ifc_dipar(int s, int d, int p, float v) :
			ITC_mesg(MT_IFC_DIPAR), _srcid(s), _divis(d), _parid(p), _value(v) {
	}
...
public:
	...
	M_ifc_retune(float f, int t) :
			ITC_mesg(MT_IFC_RETUNE), _freq(f), _temp(t) {
	}
...
public:
	...
	M_ifc_anoff(int bits) :
			ITC_mesg(MT_IFC_ANOFF), _bits(bits) {
	}
...
public:
	...
	M_ifc_preset(int type, int bank, int pres, int stat, uint32_t *bits) :
			ITC_mesg(type), _bank(bank), _pres(pres), _stat(stat) {
		if (bits)
