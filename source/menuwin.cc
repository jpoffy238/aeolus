...
X_menuwin::X_menuwin(X_window *parent, X_callback *callb,
		X_menuwin_style *style, int xp, int yp, X_menuwin_item *items) :
		X_window(parent, xp, yp, 100, 100, style->color.bg->pixel), _style(
				style), _callb(callb), _items(items), _xs(10), _ys(10), _isel(
				-1), _open(0) {
	int i, k, ys, xm;
	X_menuwin_item *P;
	XGlyphInfo G;
	...
	P = items;
	while (i < MAXITEM) {
		if (P->_bits & X_menuwin_item::SPACE)
			ys += _style->step / 2;
		_ypos[i] = ys;
		_tlen[i] = k = strlen(P->_text);
		XftTextExtentsUtf8(dpy(), _style->font, (const FcChar8 *) (P->_text), k,
				&G);
		if (xm < G.width)
			xm = G.width;
		i++;
		...
		if (P->_bits & X_menuwin_item::TITLE)
			ys++;
		if (P->_bits & X_menuwin_item::LAST)
			break;
		P++;
	}
...
void X_menuwin::bpress(XButtonEvent *E) {
	if (_open) {
		if (_isel >= 0)
			_callb->handle_callb(X_callback::MENU | SEL, this, (XEvent *) E);
		else
			_callb->handle_callb(X_callback::MENU | CAN, this, (XEvent *) E);
	} else
		_callb->handle_callb(X_callback::MENU | OPEN, this, (XEvent *) E);
}
...
void X_menuwin::drawit(int k, bool inv) {
	...
	XftColor *fg =
			(P->_bits & X_menuwin_item::MASKED) ?
					_style->color.mm :
					_style->color.fg[P->_bits & X_menuwin_item::COLMASK];

	if (XftDrawDrawable(xft()) != win())
		XftDrawChange(xft(), win());
	XSetFunction(dpy(), dgc(), GXcopy);
	XSetForeground(dpy(), dgc(), (inv) ? fg->pixel : bg->pixel);
	XFillRectangle(dpy(), win(), dgc(), DL - 2, y, _xs - DL - DR + 4,
			_style->step);
	XftDrawStringUtf8(xft(), (inv) ? bg : fg, _style->font, DL, y + d,
			(const FcChar8 *) (P->_text), _tlen[k]);
}
...
int X_menuwin::findit(int x, int y) {
	...
	for (i = 0; i < _nit; i++) {
		if ((y > _ypos[i]) && (y < _ypos[i] + _style->step - 1)
				&& (!(_items[i]._bits
						& (X_menuwin_item::MASKED | X_menuwin_item::TITLE))))
			return i;
	}
	return -1;
}
...
void X_menuwin::set_mask(unsigned m) {
	for (int i = 0; i < _nit; i++) {
		if (m & 1)
			_items[i]._bits |= X_menuwin_item::MASKED;
		m >>= 1;
	}
}
