...
Instrwin::Instrwin(X_window *parent, X_callback *callb, int xp, int yp,
		X_resman *xresm) :
		X_window(parent, xp, yp, XSIZE, YSIZE, Colors.main_bg), _callb(callb), _xresm(
				xresm), _xp(xp), _yp(yp) {
...
void Instrwin::setup(M_ifc_init *M) {
	...
	_freq_txt->x_map();
	(_dec_temp = new X_ibutton(this, this, &but2, 170, 41,
			disp()->image1515(X_display::IMG_LT), TEMP_DEC))->x_map();
	(_inc_temp = new X_ibutton(this, this, &but2, 187, 41,
			disp()->image1515(X_display::IMG_RT), TEMP_INC))->x_map();
	(_dec_freq = new X_ibutton(this, this, &but2, 170, 65,
			disp()->image1515(X_display::IMG_LT), FREQ_DEC))->x_map();
	(_inc_freq = new X_ibutton(this, this, &but2, 187, 65,
			disp()->image1515(X_display::IMG_RT), FREQ_INC))->x_map();
	but1.size.x = 60;
	but1.size.y = 20;
	(_tune_exe = new X_tbutton(this, this, &but1, 70, 100, "Retune", 0,
			TUNE_EXE))->x_map();
	(_tune_can = new X_tbutton(this, this, &but1, 135, 100, "Cancel", 0,
			TUNE_CAN))->x_map();
	...
	for (i = 0; i < M->_ndivis; i++) {
		k = DIVIS_STEP * (i + 1);
		if (M->_divisd[i]._flags & 1) {
			(D->_slid[0] = new X_hslider(this, this, &sli1, &sca_Swl, x2, y, 20,
					k))->x_map();
			(new X_hscale(this, &sca_Swl, x2, y + 20, 10))->x_map();
		} else
			D->_slid[0] = 0;
		if (M->_divisd[i]._flags & 2) {
			(D->_slid[1] = new X_hslider(this, this, &sli1, &sca_Tfr, x1, y, 20,
					k + 1))->x_map();
			(D->_slid[2] = new X_hslider(this, this, &sli1, &sca_Tmd, x1 + 160,
					y, 20, k + 2))->x_map();
			(new X_hscale(this, &sca_Tfr, x1, y + 20, 10))->x_map();
			(new X_hscale(this, &sca_Tmd, x1 + 160, y + 20, 10))->x_map();
		} else
			D->_slid[1] = D->_slid[2] = 0;
	...
	if (n2)
		add_text(x2, 5, 80, 20, "Swell", &text0, -1);
...
void Instrwin::add_text(int xp, int yp, int xs, int ys, const char *text,
		X_textln_style *style, int align) {
