...
Midiwin::Midiwin(X_window *parent, X_callback *callb, int xp, int yp,
		X_resman *xresm) :
		X_window(parent, xp, yp, XSIZE, YSIZE, Colors.main_bg), _callb(callb), _xresm(
				xresm), _xp(xp), _yp(yp), _preset(-1) {
...
void Midiwin::handle_callb(int k, X_window *W, XEvent *E) {
	...
	case BUTTON | X_button::PRESS: {
		...
		if (X->state & ShiftMask)
			...
			_callb->handle_callb(CB_MIDI_GETCONF, this, 0);
		break;
	}
...
void Midiwin::set_butt(int i) {
	if (i != _preset) {
		if (_preset >= 0)
			_bpres[_preset]->set_stat(0);
		_preset = i;
		if (_preset >= 0)
			_bpres[_preset]->set_stat(1);
	}
}

void Midiwin::add_text(int xp, int yp, int xs, int ys, const char *text,
