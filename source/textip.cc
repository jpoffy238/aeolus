...
X_textln::X_textln(X_window *parent, X_textln_style *style, int xp, int yp,
		int xs, int ys, const char *text, int xal) :
		X_window(parent, xp, yp, xs, ys, style->color.normal.bgnd), _style(
				style), _text(text) {
	XGlyphInfo K;
	...
	if (xal >= 0) {
		XftTextExtentsUtf8(dpy(), _style->font, (const FcChar8 *) _text, _len,
				&K);
		_x0 = xs - K.width - 2;
		if (xal == 0)
			_x0 /= 2;
...
void X_textln::expose(XExposeEvent *e) {
	if (e->count == 0) {
		...
		if (_len) {
			...
			XftDrawStringUtf8(xft(), _style->color.normal.text, _style->font,
					_x0, _y0, (const FcChar8 *) _text, _len);
		}
	}
}
...
X_textip::X_textip(X_window *parent, X_callback *callb, X_textln_style *style,
		int xp, int yp, int xs, int ys, int max, X_linked *back, X_linked *forw) :
		X_window(parent, xp, yp, xs, ys, style->color.normal.bgnd), X_linked(
				back, forw), _callb(callb), _style(style), _bg(
				_style->color.normal.bgnd), _fg(_style->color.normal.text), _flags(
				0), _xs(xs), _ys(ys), _txt(0), _max(max), _ic(0), _i1(0) {
	x_add_events(ExposureMask);
	...
	if (callb) {
		...
		if (!_xic)
			_xic = XCreateIC(xim(), XNInputStyle,
					XIMPreeditNothing | XIMStatusNothing, XNClientWindow, win(),
...
void X_textip::set_text(const char *txt) {
	_i1 = 0;
	if (txt)
		while ((*((unsigned char *) txt) >= ' ') && (_i1 < _max))
			_txt[_i1++] = *txt++;
	if (_flags & RIGHT) {
		_x0 = _xs - DX;
		_ic = _i1;
...
void X_textip::select(void) {
	if (_callb)
		_callb->handle_callb(X_callback::TEXTIP | BUT, this, 0);
}
...
void X_textip::bpress(XButtonEvent *e) {
	if (e->button == Button2) {
		...
		if (_callb)
			_callb->handle_callb(X_callback::TEXTIP | BUT, this, (XEvent *) e);
		xorcursor();
	}
}
...
void X_textip::spkey(XKeyEvent *e) {
	...
	case XK_Left:
		if (e->state & (ControlMask | ShiftMask))
			cbkey(k, e);
		else
			go_lt();
		break;
...
void X_textip::cbkey(int k, XKeyEvent *e) {
	_key = k;
	if (_callb)
		_callb->handle_callb(X_callback::TEXTIP | KEY, this, (XEvent *) e);
}
...
void X_textip::go_lt(void) {
	...
	if (_ic > 0) {
		for (i = _ic - 1, d = 1;
				(i > 0) && (_txt[i] >= 0x80) && (_txt[i] < 0xC0); i--, d++)
			;
		xorcursor();
		_ic -= d;
		_xc = _x0 + textwidth(0, _ic);
...
void X_textip::go_rt(void) {
	...
	if (_ic < _i1) {
		for (i = _ic + 1, d = 1;
				(i < _i1) && (_txt[i] >= 0x80) && (_txt[i] < 0xC0); i++, d++)
			;
		xorcursor();
		_ic += d;
		_xc = _x0 + textwidth(0, _ic);
...
void X_textip::redraw(void) {
	...
	XSetWindowBackground(dpy(), win(),
			(_flags & FOCUS) ? _style->color.focus.bgnd : _bg);
	XClearWindow(dpy(), win());

	if (_i1) {
		XftDrawChange(D, win());
		C = (_flags & FOCUS) ? _style->color.focus.text : _fg;
		XftDrawStringUtf8(D, C, _style->font, _x0, _y0, (const FcChar8 *) _txt,
				_i1);
	}
	if (_callb) {
...
void X_textip::xorcursor(void) {
	...
	if (_flags & FOCUS) {
		...
		XSetForeground(dpy(), gc,
				_style->color.focus.bgnd ^ _style->color.focus.line);
...
int X_textip::textwidth(int i, int n) {
	...
	if (n == 0)
		return 0;
	XftTextExtentsUtf8(dpy(), _style->font, (const FcChar8 *) (_txt + i), n,
			&K);
	return K.xOff;
}
