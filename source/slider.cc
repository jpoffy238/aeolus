...
X_slider::X_slider(X_window *parent, X_callback *callb, X_slider_style *style,
		X_scale_style *scale, int xp, int yp, int xs, int ys, int cbid) :
		X_window(parent, xp, yp, xs, ys, style->bg), _callb(callb), _style(
				style), _scale(scale), _x(0), _y(0), _i(_scale->pix[0]), _d(
				9999), _cbid(cbid), _knob(_style->knob), _mark(_style->mark) {
	x_add_events(
			ExposureMask | Button1MotionMask | ButtonPressMask
					| ButtonReleaseMask);
}
...
void X_slider::brelease(XButtonEvent *E) {
	_d = 9999;
	if (_callb)
		_callb->handle_callb(X_callback::SLIDER | STOP, this, (XEvent *) E);
}

X_hslider::X_hslider(X_window *parent, X_callback *callb, X_slider_style *style,
	...
	case ButtonPress:
		if (B->button == Button4) {
			if (domove(_scale->limit(_i + 1)) && _callb) {
				_callb->handle_callb(X_callback::SLIDER | MOVE, this,
						(XEvent *) E);
			}
...
X_vslider::X_vslider(X_window *parent, X_callback *callb, X_slider_style *style,
