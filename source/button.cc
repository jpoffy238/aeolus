...
X_button::X_button(X_window *parent, X_callback *callb, X_button_style *style,
		int xp, int yp, int cbid) :
		X_window(parent, xp, yp, style->size.x, style->size.y,
				style->color.shadow.bgnd), _style(style), _callb(callb), _xs(
				style->size.x), _ys(style->size.y), _cbid(cbid), _down(0), _stat(
				0) {
...
void X_button::redraw(void) {
	...
	case X_button_style::RAISED:
		XSetForeground(dpy(), gc,
				_down ? _style->color.shadow.dark : _style->color.shadow.lite);
		XDrawLine(dpy(), win(), gc, 0, 0, 0, _ys - 2);
		XDrawLine(dpy(), win(), gc, 0, 0, _xs - 2, 0);
		XSetForeground(dpy(), gc,
				_down ? _style->color.shadow.lite : _style->color.shadow.dark);
...
X_tbutton::X_tbutton(X_window *parent, X_callback *callb, X_button_style *style,
		int xp, int yp, const char *text1, const char *text2, int cbid) :
		X_button(parent, callb, style, xp, yp, cbid), _bg(0), _fg(0), _len1(0), _len2(
				0) {
...
void X_tbutton::set_text(const char *text1, const char *text2) {
	...
	_len2 = 0;
	if (text1)
		while (((*(unsigned char *) text1) >= ' ') && (_len1 < MAXLEN - 1))
			_text1[_len1++] = *text1++;
	if (text2)
		while (((*(unsigned char *) text2) >= ' ') && (_len2 < MAXLEN - 1))
			_text2[_len2++] = *text2++;
	_text1[_len1] = 0;
	_text2[_len2] = 0;
}
...
void X_tbutton::redraw(void) {
	...
	if (_len1) {
		...
		if (_style->type & X_button_style::ALEFT)
			x = 6;
		else {
			XftTextExtentsUtf8(dpy(), F, (const FcChar8 *) _text1, _len1, &K);
			if (_style->type & X_button_style::ARIGHT)
				x = _xs - K.width - 6;
			else
				x = (_xs - K.width) / 2;
		}
...
X_ibutton::X_ibutton(X_window *parent, X_callback *callb, X_button_style *style,
		int xp, int yp, XImage *image, int cbid) :
		X_button(parent, callb, style, xp, yp, cbid), _bg(0), _fg(0), _image(
				image) {
}
...
X_pbutton::X_pbutton(X_window *parent, X_callback *callb, X_button_style *style,
		int xp, int yp, Pixmap map0, Pixmap map1, int cbid) :
		X_button(parent, callb, style, xp, yp, cbid), _map0(map0), _map1(map1) {
	if (_map0)
		XSetWindowBackgroundPixmap(dpy(), win(), _map0);
}
...
void X_pbutton::set_stat(bool stat) {
	if (_stat != stat) {
		if (_map1)
			XSetWindowBackgroundPixmap(dpy(), win(), stat ? _map1 : _map0);
		_stat = stat;
		XClearWindow(dpy(), win());
		X_button::redraw();
	}
