...
Multislider::Multislider(X_window *parent, X_callback *callb, int xp, int yp,
		...
		X_window(parent, xp, yp, 100, 100, Colors.func_bg), _callb(callb), _bgnd(
				Colors.func_bg), _grid(grid), _mark(mark), _yc(0), _st(0), _move(
				-1), _draw(-1) {
	x_add_events(
...
void Multislider::plot_bars(void) {
	...
	for (i = 0; i < _n; i++) {
		...
		y = _yc[i];
		if (y < _yr)
			D.fillrect(x, y, x + _wx, _yr + 1);
		else
			D.fillrect(x, _yr, x + _wx, y + 1);
		x += _dx;
	}
}
