...
float Asection::_refl[16] = { 0.250f, 0.440f, 0.615f, 0.940f, 0.200f, 0.370f,
		0.560f, 0.900f, 0.230f, 0.400f, 0.500f, 0.920f, 0.280f, 0.460f, 0.690f,
		0.960f };
...
void Asection::process(float vol, float *W, float *X, float *Y, float *R) {
	...
	for (i = 0; i < PERIOD; i++) {
		t0 = _dif0.process(
				p[_offs[1]] + p[_offs[5]] + p[_offs[11]] + p[_offs[15]]
						+ 1e-20f);
		t1 = _dif1.process(
				p[_offs[0]] + p[_offs[4]] + p[_offs[10]] + p[_offs[14]]
						+ 1e-20f);
		t2 = _dif2.process(
				p[_offs[2]] + p[_offs[6]] + p[_offs[8]] + p[_offs[12]]
						+ 2e-20f);
		t3 = _dif3.process(
				p[_offs[3]] + p[_offs[7]] + p[_offs[9]] + p[_offs[13]]
						+ 2e-20f);
		p++;
		s = t0 + t1 + t2 + t3;
		_sw += 0.5f * (s - _sw);
