...
Tiface::Tiface(int ac, char *av[]) :
		_reader(this, FM_TXTIP), _stop(false), _init(true), _initdata(0), _mididata(
				0) {
	int i;
...
void Tiface::handle_ifc_elatt(M_ifc_ifelm *M) {
	...
	printf("Retuning %7s %-1s (%s)\n", _initdata->_groupd[M->_group]._label,
			_tempstr, _initdata->_groupd[M->_group]._ifelmd[M->_ifelm]._mnemo);
}
...
void Tiface::print_midimap(void) {
	...
	for (c = 0; c < 16; c++) {
		...
		if (f) {
			printf(" %2d  ", c + 1);
			if (f & 1)
				printf("keybd %-7s", _initdata->_keybdd[k]._label);
			if (f & 2)
				printf("divis %-7s", _initdata->_divisd[k]._label);
			if (f & 4)
				printf("instr");
			printf("\n");
			n++;
	...
	if (n == 0)
		printf(" No channels are assigned.\n");
}
...
void Tiface::command_s(const char *p) {
	...
	if ((sscanf(p, "%s%n", s, &n) != 1) || ((g = find_group(s)) < 0)) {
		printf("Expected a group name, ? or ??\n");
		return;
	}
	p += n;
	if (g == NGROUP + 1) {
		for (i = 0; i < _initdata->_ngroup; i++)
			print_stops_short(i);
		return;
	}
	if (g == NGROUP + 2) {
	...
	if ((sscanf(p, "%s%n", s, &n) != 1) || ((k = comm1(s)) < 0)) {
	...
	while (sscanf(p, "%s%n", s, &n) == 1) {
		i = find_ifelm(s, g);
		if (i < 0)
			printf("No stop '%s' in this group\n", s);
		else
			send_event(TO_MODEL, new M_ifc_ifelm(k, g, i));
		p += n;
	}
}
...
int Tiface::find_group(const char *p) {
	...
	for (g = 0; g < _initdata->_ngroup; g++) {
		if (!strcmp(p, _initdata->_groupd[g]._label))
			return g;
	}
	return -1;
}
...
int Tiface::find_ifelm(const char *p, int g) {
	...
	for (i = 0; i < n; i++) {
		if (!strcmp(p, _initdata->_groupd[g]._ifelmd[i]._mnemo))
			return i;
	}
	return -1;
}
