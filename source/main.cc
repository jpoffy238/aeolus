...
static void help(void) {
	fprintf(stderr, "\nAeolus %s\n\n", VERSION);
	fprintf(stderr,
			"  (C) 2003-2008 Fons Adriaensen  <fons@kokkinizita.net>\n");
...
#else
	fprintf(stderr,
			"  -N <name>          Name to use as JACK and CoreMIDI client [aeolus]\n");
#endif
...
#endif
	model = new Model(&comm_queue, &midi_queue, audio->midimap(),
			audio->appname(), S_val, I_val, W_val, u_opt);
	imidi = new Imidi(&note_queue, &midi_queue, audio->midimap(),
			audio->appname());
	slave = new Slave();
	iface = new xiface::Xiface(ac, av);
